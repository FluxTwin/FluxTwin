# app.py — FluxTwin Enterprise (Executive Dashboard + Branding + Theme + AI + Holt-Winters + Pro PDF)
...
def themed_css(mode: str = "Dark") -> str:
    dark = """
    :root {
      --ft-bg: #0b1220; --ft-card: #10182a; --ft-accent: #4ea1ff;
      --ft-text: #ecf2ff; --ft-muted: #94a3b8; --ft-border: rgba(255,255,255,0.08);
    }"""
    light = """
    :root {
      --ft-bg: #f8fafc; --ft-card: #ffffff; --ft-accent: #2563eb;
      --ft-text: #0f172a; --ft-muted: #64748b; --ft-border: rgba(15,23,42,0.08);
    }"""
    ...
    .exec-grid .stMetric > div > div {{ font-size: 16px }}
...
# ---------- FORECAST (Holt-Winters + robust fallback) ----------
def forecast_daily(df: pd.DataFrame, horizon_days: int = 7) -> pd.DataFrame:
    ...
    Robust when there is no history: starts forecasting from (last_ts + 1 day) or today.
    """
    s = daily_series(df)
    # determine start anchor
    if not df.empty and "timestamp" in df.columns and pd.to_datetime(df["timestamp"], errors="coerce").notna().any():
...
def rule_based_advice(profile: dict, forecast_df: pd.DataFrame | None) -> dict:
    ...
    if usage in ("office","hotel"):
        tips += [
            "Shift non-critical loads to off-peak hours (server backups, laundry).",
            "HVAC setpoints: +1 °C cooling / −1 °C heating outside peak hours.",
            "Occupancy sensors & daylight dimming in corridors/meeting rooms.",
    ...
    if has_pv:
        tips += [
            "Pre-heat water/space during PV peak (11:00–15:00).",
            "Consider 3–5 kWh battery to shave evening peaks.",
        ]
    tips += [
...
def ai_advice_openai(profile: dict, kpis: dict, fc_df: pd.DataFrame, api_key: str, model: str = "gpt-4o-mini") -> dict:
    ...
    payload = {
        "profile": profile,
        "kpis": kpis,
        "forecast": fc_df[["date", "forecast_kwh"]]
...
# AI Advisor (tips + expected savings)
...
with cA:
    st.markdown(f'<div class="ft-card"><div class="ft-kpi">Current period — cost</div><div class="ft-kpi-val">{est_cost_now:,.2f} €</div></div>', unsafe_allow_html=True)
with cB:
    st.markdown(f'<div class="ft-card"><div class="ft-kpi">Forecast — cost (no action)</div><div class="ft-kpi-val">{cost_no_action:,.2f} €</div></div>', unsafe_allow_html=True)
with cC:
    st.markdown(f'<div class="ft-card"><div class="ft-kpi">Forecast — cost (after actions)</div><div class="ft-kpi-val">{cost_after:,.2f} €</div></div>', unsafe_allow_html=True)
with cD:
    st.markdown(f'<div class="ft-card"><div class="ft-kpi">Potential savings</div><div class="ft-kpi-val">{savings_eur:,.2f} €  ({savings_pct*100:.1f}%)</div></div>', unsafe_allow_html=True)
...
with colA: st.metric("Total consumption (kWh)", f"{total_now:,.2f}")
with colB: st.metric("Average sample (kWh)", f"{avg_now:,.2f}")
with colC: st.metric("Max sample (kWh)", f"{max_now:,.2f}")
...
with col2:
    st.markdown(
        f'<div class="ft-card"><div class="ft-kpi">Forecast total</div>'
        f'<div class="ft-kpi-val">{fc_total_kwh:,.0f} kWh</div>'
        f'<div class="ft-kpi" style="margin-top:8px;">Estimated cost (no action)</div>'
...
# ---------- 3) AI ADVISOR ----------
st.markdown("### 3) AI Advisor — next 7 days action plan")
for t in tips_list:
    st.markdown(f"- {t}")

# ---------- 4) FORECAST — AFTER ACTIONS ----------
st.markdown("### 4) Forecast — After actions")
fc_after = fc_df.copy()
...
with colX:
    st.metric("Estimated cost (no action)", f"{cost_no_action:,.2f} €")
with colY:
    st.metric(f"Estimated cost (after actions, -{savings_pct*100:.1f}%)", f"{cost_after:,.2f} €")
with colZ:
    st.metric("Estimated savings", f"{savings_eur:,.2f} €")
...
with colR:
    ...
    fig3 = px.line(cum, x="date", y=[
        "Cumulative cost — No action (€)",
        "Cumulative cost — After actions (€)"
    ], title="Cumulative cost curves (€)")
    st.plotly_chart(fig3, use_container_width=True)

with st.expander("Daily forecast table — No action vs After actions (kWh & €)", expanded=False):
    tv = fc_after.copy().rename(columns={
        "date":"Date",
...
# ---------- 5) EXPORT (PDF via utils/pdf_report) ----------
st.markdown("### 5) Export")
if st.button("Generate Executive PDF"):
    pdf_path = pdf_report.create_report(
        df=data,
        advisor_text_or_list=tips_list,
